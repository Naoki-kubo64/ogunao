...
test.describe('Story Mode Testing', () => {
    test('Story Mode UI Test', async ({ page }) => {
        ...
        if (!storyVisible) {
            ...
            // Try to manually show the story screen for testing
            await page.evaluate(() => {
            ...
            console.log('🔧 Manually triggered story mode display');
            await page.waitForTimeout(1000);
            
            // Re-check visibility
            const storyVisibleAfterManual = await storyScreen.isVisible();
            console.log('🔍 Story screen visible after manual trigger:', storyVisibleAfterManual);
        }
        
        // Take another screenshot after manual trigger
        await page.screenshot({ path: 'story_test_03_after_manual.png', fullPage: true });
        console.log('📸 Screenshot saved: story_test_03_after_manual.png');
        
        // Test story mode UI elements if visible now
        const storyVisibleFinal = await storyScreen.isVisible();
        if (storyVisibleFinal) {
            ...
            const titleVisibleAfterBack = await titleScreen.isVisible();
            console.log('🔍 Title screen visible after back button:', titleVisibleAfterBack);
            
            await page.screenshot({ path: 'story_test_05_back_to_title.png', fullPage: true });
            console.log('📸 Back to title screenshot saved: story_test_05_back_to_title.png');
        } else {
            console.log('❌ Story screen could not be made visible - UI test skipped');
        }
        
        // Take final screenshot
        await page.screenshot({ path: 'story_test_04_final_state.png', fullPage: true });
        console.log('📸 Final screenshot saved: story_test_04_final_state.png');
